...
- **Тот же 2008 даёт и конечный размер λ₂(N)** (дословно, Sec. I/Table 1):
  `λ₂(N) ∼ (ln N)^{−2}` при `q_m ≤ 2` и `λ₂(N) − λ_c ∼ (ln ln N)^{−2}` при
  `q_m > 2`. ⇒ наш «power law `n^{−α}`» — **эффективный** показатель на n ≤ 10⁵;
  истинная асимптотика **логарифмическая**. Направление «finite-size scaling» (A)
  тоже закрыто. Нормализованный случай они предвидели («main conclusions are also
  valid if the escape rate from a vertex is fixed»).

**ФИНАЛЬНЫЙ ВЫВОД (2026-09-23): тема закрыта.** Ядро (min degree → щель; хвост
вторичен), конечный размер и качественная картина локализации опубликованы в
2003–2008. Нового результата в этой нише получить нельзя. Проект остановлен.
Все полученные данные/код сохранены как воспроизведение и численная детализация
известного; при желании — как отправная точка для другой темы.

Старые заметки ниже сохранены для истории.
