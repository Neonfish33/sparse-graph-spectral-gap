...
1. Реализовать генерацию графов для четырёх ансамблей.
2. Реализовать sweep cut для φ_periph, φ_mod, Δ, ρ_*.
3. Провести численные эксперименты для n = 10³, 10⁴, 10⁵.
4. Построить фазовую диаграмму в (log Δ, ρ_*).
5. Проверить гипотезу λ₂ ↔ min(φ_periph, φ_mod).
6. Сформулировать эмпирические гипотезы о порядке λ₂ в каждом режиме.
7. Попробовать аналитический вывод для простейшего случая (смешанно-регулярный граф, два типа степеней).
8. Оформить результаты как препринт (arXiv: math.SP, math.CO, q-bio.MN).
